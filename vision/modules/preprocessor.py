    ...
    def process(self, *images):
        ...
        for mat in images:
            ...
            if self.options_dict['PPX_color_correction'].value:
                mat = balance(mat)
            if self.options_dict['PPX_r_bias'].value !=\
               self.options_dict['PPX_r_bias'].default:
                bgr_split = cv2.split(mat)
                bgr_split[2] = cv2.add(self.options_dict['PPX_r_bias'].value, bgr_split[2])
                mat = cv2.merge(bgr_split)
            if self.options_dict['PPX_g_bias'].value !=\
               self.options_dict['PPX_g_bias'].default:
                bgr_split = cv2.split(mat)
                bgr_split[1] = cv2.add(self.options_dict['PPX_g_bias'].value, bgr_split[1])
                mat = cv2.merge(bgr_split)
            if self.options_dict['PPX_b_bias'].value !=\
               self.options_dict['PPX_b_bias'].default:
                bgr_split = cv2.split(mat)
                bgr_split[0] = cv2.add(self.options_dict['PPX_b_bias'].value, bgr_split[0])
                mat = cv2.merge(bgr_split)
            if self.options_dict['PPX_contrast'].value !=\
               self.options_dict['PPX_contrast'].default:
                temp = mat * self.options_dict['PPX_contrast'].value
                mat = numpy.clip(temp, 0., 255.).astype(numpy.uint8)
            if self.options_dict['PPX_brightness'].value !=\
               self.options_dict['PPX_brightness'].default:
            ...
            if self.options_dict['PPX_gaussian_blur'].value:
                mat = cv2.GaussianBlur(mat,
                        (self.options_dict['PPX_gaussian_blur_kernel'].value * 2 + 1,
                         self.options_dict['PPX_gaussian_blur_kernel'].value * 2 + 1),
                        0)
            if self.options_dict['PPX_gaussian_noise'].value !=\
               self.options_dict['PPX_gaussian_noise'].default:
                ...
                mat = cv2.dilate(mat,
                        cv2.getStructuringElement(cv2.MORPH_ELLIPSE,
                            (self.options_dict['PPX_dilate_kernel'].value * 2 + 1,
                             self.options_dict['PPX_dilate_kernel'].value * 2 + 1)))
            if self.options_dict['PPX_rotate'].value !=\
               self.options_dict['PPX_rotate'].default:
            ...
            if self.options_dict['PPX_resize_ratio'].value !=\
               self.options_dict['PPX_resize_ratio'].default:
                mat = cv2.resize(mat,
