...
int process_frame(unsigned char *arr, size_t height, size_t width, size_t depth) {
    bool EQUALIZE_RGB = true; // Removes color cast from image, but may cause
                              // washing out if image is entirely one color
    bool RGB_CONTRAST_CORRECT = false; // Faster, but tends to overcompensate,
                                       // resulting in opposite color cast
    bool HSV_CONTRAST_CORRECT = false; // Faster, but shifts color cast, due to
                                      // calculation of value
    bool HSI_CONTRAST_CORRECT = true; // Best quality, but slower
    bool RGB_EXTREMA_CLIPPING = true;
    bool ADAPTIVE_CAST_CORRECTION = true; // If false, then the same gain is used
                                          // for the same component of every pixel
                                          // If true, then the gain decreases as
                                          // the pixel value nears 255 (max)
    ...
    // Equalize RGB
    if (EQUALIZE_RGB) {
        //size_t horizontal_blocks = 16;
        //size_t vertical_blocks = 16;
        //size_t block_width = width / horizontal_blocks;
        //size_t block_height = height / vertical_blocks;
        ...
        //    ++vertical_blocks;
        //}
        //for (size_t block_y = 0; block_y < vertical_blocks; ++block_y) {
        //    for (size_t block_x = 0; block_x < horizontal_blocks; ++block_x) {
        //        printf("block_x: %ld\n", block_x);
        //        printf("block_y: %ld\n", block_y);
        ...
        if (r_avg > g_avg && r_avg > b_avg) {
            double g_gain = r_avg / g_avg;
            double b_gain = r_avg / b_avg;
            for (size_t i = 0; i < channel_size; ++i) {
                if (ADAPTIVE_CAST_CORRECTION) {
                    g_channel[i] = constrain(g_channel[i] * (sqrt((255. - g_channel[i]) / 255.) * (g_gain - 1.) + 1.), 0, 255);
                    b_channel[i] = constrain(b_channel[i] * (sqrt((255. - b_channel[i]) / 255.) * (b_gain - 1.) + 1.), 0, 255);
                }
                else {
                    g_channel[i] = constrain(g_channel[i] * g_gain, 0, 255);
                    b_channel[i] = constrain(b_channel[i] * b_gain, 0, 255);
                }
            }
        }
        // Green color cast
        else if (g_avg > r_avg && g_avg > b_avg) {
            double r_gain = g_avg / r_avg;
            double b_gain = g_avg / b_avg;
            for (size_t i = 0; i < channel_size; ++i) {
                if (ADAPTIVE_CAST_CORRECTION) {
                    r_channel[i] = constrain(r_channel[i] * (sqrt((255. - r_channel[i]) / 255.) * (r_gain - 1.) + 1.), 0, 255);
                    b_channel[i] = constrain(b_channel[i] * (sqrt((255. - b_channel[i]) / 255.) * (b_gain - 1.) + 1.), 0, 255);
                }
                else {
                    r_channel[i] = constrain(r_channel[i] * r_gain, 0, 255);
                    b_channel[i] = constrain(b_channel[i] * b_gain, 0, 255);
                }
            }
        }
        // Blue color cast
            ...
            double r_gain = b_avg / r_avg;
            double g_gain = b_avg / g_avg;
            for (size_t i = 0; i < channel_size; ++i) {
                if (ADAPTIVE_CAST_CORRECTION) {
                    r_channel[i] = constrain(r_channel[i] * (sqrt((255. - r_channel[i]) / 255.) * (r_gain - 1.) + 1.), 0, 255);
                    g_channel[i] = constrain(g_channel[i] * (sqrt((255. - g_channel[i]) / 255.) * (g_gain - 1.) + 1.), 0, 255);
