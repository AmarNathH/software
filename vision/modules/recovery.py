...
class Recovery(ModuleBase):
    ...
    def tubes(self, mat, lab, ycrcb, lab_sp, hsv_sp):
        results = [shm._eval('recovery_tube{}'.format(i)).get() for i in range(4)]
        ...
        eroded1 = cv2.morphologyEx(thresh1, cv2.MORPH_OPEN, morph_kernel)

        self.post('tube thresh0', thresh0)
        self.post('tube eroded1', eroded1)

        # Find contours on single binary image to prevent redundant contours
        threshed = cv2.bitwise_or(thresh0, thresh1)
    ...
    def table(self, mat, lab, ycrcb):
        ...
        if len(infos) > 0:
            info = max(infos, key=lambda x: x['area'])

            # Pass threshed instead of eroded since it will appear dilated when
            # inverted
            self.ellipses(mat, lab, ycrcb, info['contour'], threshed, bgr_sp, morph_kernel)

    def ellipses(self, mat, lab, ycrcb, table_contour, table_thresh, bgr_sp, morph_kernel):
        results = [shm._eval('recovery_ellipse{}'.format(i)).get() for i in range(4)]
        for result in results:
            result.visible = False
        ...
        thresh0 = np.invert(table_thresh) # Should get all ellipses except blue
        _, thresh1 = cv2.threshold(
            bgr_sp[2],
            self.options['blue_ellipse_thresh'],
            255,
            cv2.THRESH_BINARY_INV,
        )
        self.post('blue ellipse thresh', thresh1)
