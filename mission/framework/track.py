...
class Matcher:
    ...
    def match(self, observations):
        return self._match(observations)

    def _match(self, observations, pattern_changed=False):
        ...
        if (tracked_count != prev_tracked_count or pattern_changed) and \
                tracked_count >= len(self._pattern):
    ...
    def update_pattern(self, pattern):
        ...
        return self._match([t.latest_obs for t in self._trackers], pattern_changed=True)
