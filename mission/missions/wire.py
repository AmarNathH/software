...
from mission.framework.primitive import Zero, Log, Fail
...
class AlignFore(Task):
    ...
    def on_run(self, vision, *args, **kwargs):
        if(vision.bottom is not None and ((vision.bottom.x1 <5) ^ (abs(vision.bottom.x2 -vision.cam_width) < 5))):
            self.finish()
        if vision.bottom is not None and (abs(vision.bottom.y1 - vision.cam_height) <5 or abs(vision.bottom.y2 - vision.cam_height) < 5):
            self.finish()
...
def checkAligned(vision):
    a = abs(x_ratio(vision) - .5) < .03
    b = abs(bbar_angle(vision)) < 5
    c = abs(bbar_width_ratio(vision)- BBAR_RATIO_TARGET) < .05
    print("x_ratio correct: {}".format(a))
    print("bbar_angle correct: {}".format(b))
    print("bbar_width_ratio correct: {}".format(c))
...
class AlignChannel(Task):
    def on_first_run(self, vision, *args, **kwargs):
        align_fore = Conditional(
            AlignFore(vision),
            on_fail=Fail(Log('Lost gate, aborting align')),
        )
        self.depth = DepthRestore()
        ...
        self.use_task(
            Sequential(
                Log("DEPTH"),
                self.depth,
                While(lambda: self.align_task(vision), check),
            )
            )


    def align_task(self, vision):
        return Sequential(
            Log('Align Heading'),
            AlignHeading(vision),
