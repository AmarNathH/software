...
AMLANS = [
    Amlan(
        downcam_offset=(0.25019, -0.18446),
        name='left',
    ...
    Amlan(
        downcam_offset=(0.25364, 0.131572),
        name='right',
...
class Vision(Task):
    class ColorObs(Observation):
        def __init__(self, shm_obj, camera_coord, heading_inv_angle, complex_color):
        ...
        def match_distance_sq(self, obs):
            # Only match along color dimensions since only these are known at this
            # point
            return self.complex_color.match_distance_sq(obs.complex_color)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initially pattern-match tubes and ellipses by color (later by pos/angle)
        ...
        def color_pattern(color_type_func):
            ...
            for i, c in enumerate(constants.colors):
                ...
                pattern.append(Match(i, self.ColorObs(None, camera_coord, angle, complex_color)))
    ...
    def pull_colored_objs(self, name, obs_func):
        ...
        for obj in self.read_shm_objs(name, len(constants.colors)):
            ...
            obs.append(obs_func(obj, camera_coord, angle, color))
...
class Recovery(Task):
    def on_first_run(self, reset_state=True, *args, **kwargs):
        if reset_state:
            for loc in ['table', 'tower']:
                ...
                for i in range(len(constants.colors)):
        ...
        self.use_task(Conditional(
            Defer(
                Sequential(
                    ...
                    Log('Initially retracting Amlans'),
                    AMLANS[0].Retract(),
                    AMLANS[1].Retract(),
...
class TryRecovery(Task):
    def on_first_run(self, vision, *args, **kwargs):
        ...
        def Iteration(first_run=False):
            return Sequential(
                # Classification is buggy and unnecessary for now
                # Sequential(
        ...
        self.task = Sequential(
            ...
            While(
                Iteration,
                lambda: sum(get_shm_array(shm.recovery_world_tower, 'has_tube')) > 0,
            ),
        )
    ...
    def on_run(self, vision, *args, **kwargs):
        try:
            ...
            self.loge('Global timeout, done trying recovery T_T')
    ...
    def on_first_run(self, vision, *args, **kwargs):
        ...
        if tower.know_pos:
            ...
            Log('Returning to tower position ({}, {})'.format(tower.north, tower.east)),
                GoToPosition(tower.north, tower.east, optimize=False),
            )
...
class AlignAmlan(Task):
    ...
    """
    def on_first_run(self, obj_func, amlan, align_depth_task, blind=False, *args, **kwargs):
    ...
    def on_run(self, obj_func, amlan, *args, **kwargs):
...
class GrabTube(Task):
    def on_first_run(self, vision, tube, amlan):
        self.use_task(Sequential(
            ...
            Altitude(constants.tube_grab_altitude),
            ...
            Log('Plucking tube'),
            Succeed(Timeout(RelativeToInitialDepth(-0.25, error=0.10), 10)),
        ))
...
class Altitude(Task):
    def on_first_run(self, altitude, *args, **kwargs):
        def desired_depth():
            ...
            return shm.kalman.depth.get() + current_altitude - altitude

        self.use_task(Depth(desired_depth))
    ...
    def on_first_run(self, vision, *args, **kwargs):
        ...
        self.use_task(Sequential(
            ...
            Sequential(
                ...
                GoToPosition(table.north, table.east, optimize=False),
            ) if table.know_pos else NoOp(),

            Log('Searching for table'),
            MasterConcurrent(
                IdentifyObjects(lambda: vision.ellipses),
                SearchWithGlobalTimeout()
            ),
    ...
    def on_first_run(self, vision, amlan, blind=False):
        self.use_task(Sequential(
            ...
            Sequential(
                ...
                FastDepth(constants.drop_depth),
            ) if not blind else NoOp(),
            Timer(1),
            ...
            amlan.Retract(),
            Timer(1),
            FunctionTask(lambda: amlan.shm_tube.set(-1)),
...
class OptimalRecovery(Task):
    ...
    def on_first_run(self):
        ...
        # TODO @AlexO Update when we've made progress! (first successful center
        # on tower? first grabbed tube confirmed?)
...
vision = Vision()
def recovery(*args, **kwargs): return MasterConcurrent(Recovery(vision, *args, **kwargs), vision)
sim_recovery = lambda: SimulatedTask(recovery())
altitude_until_stop = lambda: AltitudeUntilStop(1)

def test_fire(): return Sequential(
        AMLANS[0].Extend(),
        Timer(2),
        AMLANS[0].Retract(),
    )
...
def classify_tubes():
    return Concurrent(
        Sequential(Timer(2), FunctionTask(vision.classify_tubes)),
        vision,
    )

def classify_ellipses():
    return Concurrent(
        Sequential(Timer(2), FunctionTask(vision.classify_ellipses)),
        vision,
    )
...
def after_two_grabs():
    ...
    return MasterConcurrent(PlaceTubes(vision), vision)

def extend_left(): return AMLANS[0].Extend()
def retract_left(): return AMLANS[0].Retract()
def extend_right(): return AMLANS[1].Extend()
def retract_right(): return AMLANS[1].Retract()

def grad_depth(): return GradualDepth(1)
