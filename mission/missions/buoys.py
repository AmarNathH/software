...
#from mission.constants.config import BUOY_SEARCH_DEPTH
...
BUOY_SEARCH_DEPTH = 1.95 
BUOY_OVER_DEPTH = 1

#BUOY_RIGHT_TO_REACH = [False, True, False] #YELLOW RED GREEN
BUOY_RIGHT_TO_REACH = [False, True, False] #RED YELLOW GREEN
#BUOY_RIGHT_TO_REACH = [True, False, True] #GREEN YELLOW RED
...
class DepthRestore(Task):
    ...
    def __init__(self, depth=None, *args, **kwargs):
        ...
        if depth is None:
            self.start_depth = BUOY_SEARCH_DEPTH
        else:
...
class LocateBuoySurge(Task):
    ...
    """
    def __init__(self, validator, forward=True, timeout=5, *args, **kwargs):
        """
...
class LocateFirstBuoy(Task):
    ...
    Surges until it sees either the desired buoy or the Middle buoy, then strafes.
    """
    def __init__(self, validator, forward=True, right=True, middle=yellow_buoy_results, *args, **kwargs):
        """
        validator - a function that returns True when a buoy is found and False
            otherwise.
        forward - determines whether the submarine should move forward or
        ...
        if self.middle_check:
            ...
            self.task_classes = [lambda: LocateBuoySurge(validator, forward),
    ...
    def middle_check(self):
        ...
        return self.seen_frames_checker.check(self.middle_buoy.probability.get() != 0) & (self.middle_buoy.percent_frame.get() > 1)

class LocateBuoy(Task):
    """
    Locates a buoy using LocateBuoyBySpinning and LocateBuoySurge
    """
    def __init__(self, validator, forward=True, checkBehind=False, *args, **kwargs):
        """
        validator - a function that returns True when a buoy is found and False
            otherwise.
        forward - determines whether the submarine should move forward or
        ...
        if checkBehind:
            self.task_classes.insert(0, lambda: LocateBuoySurge(validator, False))
        self.tasks = []
        self.task_index = 0
        self.TIMEOUT = 60
...
class LocateAdjacentBuoy(Task):
    ...
    def __init__(self, validator, right=True, changeDir=True, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.validator = validator
        self.task_classes = [lambda: LocateBuoyStrafe(validator, right=right)]
        if changeDir:
            self.task_classes.append(lambda: LocateBuoyStrafe(validator, right= not right))
...
class AlignTarget(Task):
    ...
    target is visible
    """
    def __init__(self, validator, locator_task, target_coords, vision_group, heading_task, forward_target_p=0.001, *args, **kwargs):
        """
        ...
        def get_center():
            #return (vision_group.frame_width.get() / 2, vision_group.frame_height.get() / 2)
        ...
        self.target_task = ForwardTarget(target_coords, target=get_center,
                px=forward_target_p, dx=forward_target_p/2,
                py=forward_target_p, dy=forward_target_p/2, deadband=(30, 30))
...
class BuoyRam(Task):
    ...
    def __init__(self, location_validator, target_coordinates, vision_group,
            collision_validator, ram_concurrent_task=NoOp(), first_buoy=False, right=True, *args, **kwargs):
        """
        ...
        if first_buoy:
            self.locator_task_a = LocateBuoySurge(self.location_validator, forward=True)
        else:
            self.locator_task_a = LocateAdjacentBuoy(self.location_validator, right=right)
        self.locator_task_b = LocateBuoySurge(self.location_validator, forward=False)
        ...
        self.forward_task = DirectionalSurge(2, .2)
        self.retreat_task = DirectionalSurge(5, -1)
...
class Buoy(Task):
    ...
    buoy
    """
    def __init__(self, buoy, right=True, first_buoy=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.align_task = AlignTarget(self.location_validator,
                ...
                (self.buoy.r_side_x.get, self.buoy.center_y.get), self.buoy, self.heading_task)
        self.ram_task = BuoyRam(self.location_validator,
                (self.buoy.r_side_x.get, self.buoy.center_y.get), self.buoy,
                self.collision_validator, self.align_task, right=right, first_buoy=first_buoy)
...
class AllBuoys(Task):
    ...
    def on_first_run(self):
        ...
        self.depth_task = DepthRestore()
        self.heading_task = HeadingRestore()
        self.up_task = DepthRestore(BUOY_OVER_DEPTH)
        self.over_task = DirectionalSurge(7, .4, compensate=True)
        self.heading_task = HeadingRestore()
        self.task = Sequential(
            ...
            Buoy(red_buoy_results, first_buoy=True, right=BUOY_RIGHT_TO_REACH[0]),
            #self.depth_task,
            ...
            SeqLog("Looking for green buoy"), LocateBuoyStrafe(lambda: location_validator(green_buoy_results), right=BUOY_RIGHT_TO_REACH[1], timeout=3),
            Buoy(green_buoy_results, right=BUOY_RIGHT_TO_REACH[1]),
            #self.depth_task,
            SeqLog("Looking for yellow buoy"), LocateBuoyStrafe(lambda: location_validator(yellow_buoy_results), right=BUOY_RIGHT_TO_REACH[2], timeout=2),
            Buoy(yellow_buoy_results,  right=BUOY_RIGHT_TO_REACH[2]),
            SeqLog("Rising to Over depth"), self.up_task,
            SeqLog("Going over buoys"), self.over_task
        )
